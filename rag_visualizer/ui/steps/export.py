...
from rag_visualizer.services.export import (
    ExportConfig,
...
def render_export_step() -> None:
    ...
    with st.container(border=True):
        ...
        with cols[1]:
            st.markdown(f"**Chunk Size:** {summary['chunk_size']}")
...
Configuration exported from RAG Visualizer:
...
def run_pipeline(file_path: str, query: str = None):
    ...
    if query:
        ...
        similarities = embeddings @ query_embedding  # Cosine similarity
        top_indices = similarities.argsort()[::-1][:5]

        result["query"] = query
        result["top_chunks"] = [(chunks[i], similarities[i]) for i in top_indices]
